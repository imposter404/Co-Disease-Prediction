...
def patient(symptom):
    global test
    print(symptom)
    symptoms_list=list(df.columns)
    symptoms_list.pop()
    test=np.zeros(len(symptoms_list),dtype=int)
    test=pd.DataFrame([test],columns=symptoms_list)
    for x in symptom:
        test[x]=1
    # print(test)
    test=[list(test.iloc[0])]
...
def Disease_percentage():
    ...
    print(final)
    final={"disease":final['Disease'].to_list(),"percentage":final['%'].to_list()}
    print('---------------------------------------')
    ...
    if len(apriori_final)!=0: #return value
        ...
        print(final_apriori)
        temp1=final_apriori['Disease'].to_list()
        temp1=[item for sublist in temp1 for item  in sublist]
        temp2=final_apriori['%'].to_list()
        final_apriori={"disease":temp1,"percentage":temp2}
    else:
        final_apriori="none"
    print('---------------------------------------')
...
# SVM()
# Apriori()
# patient(['itching','skin_rash','nodal_skin_eruptions'])